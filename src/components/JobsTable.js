    ...
    const getData = async () => {
      try {
        const res = await axios.get(
          `/jobs?limit=10&page=${getPageNumberFromUrl()}&location=${jobLocation}&type=${jobType}&department=${department}`
        );
  ...
  return (
    <Container>
      <DropdownButtonsBar
        ...
        updateFiltering={updateFiltering}
        filtering={filtering}
      />
          ...
          <ul className="pagination">
            <li className="page-item">
              <span className="page-link text-dark border-white px-0 mx-0">
                {totalCount > 0 ? (
                  <>
                    Page {pageNum} of {Math.ceil(totalCount / 10)}{" "}
                  </>
                ) : (
                  <>Sorry, we don't currently have any open positions.</>
                )}
              </span>
            </li>
          </ul>
...
const DropdownButton = ({
  ...
  const handleChange = (item) => {
    if (title === "Job Type")
      updateJobType(item === "All Job Types" ? "" : item);
    else if (title === "Location")
      updateLocation(item === "All Locations" ? "" : item.split(",")[0]);
    else if (title === "Department")
      updateDepartment(item === "All Departments" ? "" : item);
